...
public class PinchZoom : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if(controlCamera && useBounds && cam.orthographic)
            CameraInBounds();
    }
    ...
    void UpdateWithTouch()
    {
        ...
        else if (touchCount == 2)
        {
            ...
            {
                OnPinch((touch0.position + touch1.position) * .5f, previousDistance, currentDistance);
            }
        }
        else
        {
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance)
    {
        ...
        if (!(controlCamera && cameraControlEnabled)) return;
        if (cam.orthographic)
        {
    ...
    public void CancelCamera() => cameraControlEnabled = false;

    void CameraInBounds()
    {
        cam.orthographicSize = Mathf.Min(cam.orthographicSize, ((boundMaxY - boundMinY) * .5f) - 0.001f);
        cam.orthographicSize = Mathf.Min(cam.orthographicSize, (Screen.height * (boundMaxX - boundMinX) / (2 * Screen.width)) - 0.001f);

        Vector2 margin = cam.ScreenToWorldPoint((Vector2.up * Screen.height * .5f) + (Vector2.right * Screen.width * .5f)) - cam.ScreenToWorldPoint(Vector2.zero);
